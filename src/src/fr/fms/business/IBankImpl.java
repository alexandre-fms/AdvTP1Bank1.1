...
import src.fr.fms.entities.Transfert;
import src.fr.fms.entities.withdrawal;
...
public class IBankImpl implements IBank {
	...
	/**
	 * Vérifie l'existance d'un compte via le numéro donné par l'utilisateur
	 * @param accountId le numéro de compte à vérifier
	 * @return retourne vrai si le compte existe
	 */
	public boolean isAccountExists(Long accountId) {
		Account account = accounts.get(accountId);
		if(account == null) return false; else return true;
	}
